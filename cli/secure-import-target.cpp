...
)
{
    ...
    ErrorCode e = hal.generateKey(params, out_wrapping_blob, kc, &keymint_cert_chain);
    if (e != ErrorCode::OK) {
    ...
    if (hal.getVersion() >= kmhal::HAL_KEYMINT_1_0) {
        if (keymint_cert_chain.size() == 0) {
            std::cerr << "Invalid cert chain returned by KeyMint generateKey" << std::endl;
            return 1;
        }

        if (extract_subject_pubkey(out_wrapping_pubkey, keymint_cert_chain[0])) {
        ...
        if (out_cert_chain != nullptr)
            *out_cert_chain = keymint_cert_chain;
...
)
{
    ...
    ErrorCode e = hal.importWrappedKey(in_wrapped_data, in_wrapping_blob,
                in_masking_key, params, 0, 0, out_key_blob, kc, out_opt_keymint_cert_chain);
    if (e != ErrorCode::OK) {
