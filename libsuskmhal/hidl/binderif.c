...
enum kmhal_hidl_binder_fail_flags {
...
struct kmhal_hidl_binder_ctx {
     ...
     * to handle transactions (`enum kmhal_hidl_binder_fail_flags`) */
    u32 fail_flags;
...
struct transaction_data {
    ...
    union {
        struct td_transaction_sg {
            struct kmhal_hidl_binder_tr_sg_args_out *out_data;
...
struct kmhal_hidl_binder_ctx * kmhal_hidl_binder_open_dev(const char *dev_path)
{
    ...
    ret->ioctl_lock = (pthread_mutex_t){};
    ...
    ret->td_map = mmap(NULL, KMHAL_HIDL_BINDER_TD_READ_BUF_SIZE,
...
{
    struct kmhal_hidl_binder_transaction *ret = NULL;

    ret = malloc(sizeof(struct kmhal_hidl_binder_transaction));
    if (ret == NULL) {
...
)
{
    ...
    struct kmhal_hidl_binder_tr_sg_args_in *const i = &arg->in_data;
    ...
    arg->out_reply.status = KMHAL_HIDL_BINDER_TR_SG_PENDING;
}
...
{
    if (!kmhal_hidl_binder_ctx_ok(ctx) || txn_p == NULL || *txn_p == NULL ||
            (*txn_p)->buf == NULL)
    {
        s_log_error("Invalid parameters!");
        return -1;
    ...
    int r_;
    u32 status = 0;
    struct kmhal_hidl_binder_transaction *txn = *txn_p;
    ...
    *txn_p = NULL; /* Prevent any possible use early, just in case */
    ...
    kmhal_hidl_binder_transaction_destroy(&txn);
...
void kmhal_hidl_binder_close(struct kmhal_hidl_binder_ctx **ctx_p)
{
    ...
    if (ctx->td_map != MAP_FAILED) {
        if (munmap(ctx->td_map, KMHAL_HIDL_BINDER_TD_READ_BUF_SIZE))
...
static void reset_transaction_output_data(VECTOR(struct transaction_data) data)
{
    for (u32 i = 0; i < vector_size(data); i++) {
        ...
        case TRANSACTION_SG:
            ...
            curr->data.tr_sg.out_data->status = KMHAL_HIDL_BINDER_TR_SG_FAILED;
        ...
        bool fail)
{
    for (u32 i = 0; i < vector_size(data); i++) {
        ...
        case TRANSACTION_SG:
            ...
            {
                curr->data.tr_sg.out_data->status = KMHAL_HIDL_BINDER_TR_SG_OK;
            } else {
                memset(curr->data.tr_sg.out_data, 0,
                        sizeof(struct kmhal_hidl_binder_tr_sg_args_out));
                curr->data.tr_sg.out_data->status =
                    KMHAL_HIDL_BINDER_TR_SG_FAILED;
            }
...
static u32 do_write_read_ioctl_loop(int fd,
        struct kmhal_hidl_binder_transaction *txn)
{
    u8 proto_read_buf[KMHAL_HIDL_BINDER_PROTO_READ_BUF_SIZE] = { 0 };
    u32 ret = 0;
    ...
    struct binder_write_read bwr = {
        ...
        .read_size = do_receive ? KMHAL_HIDL_BINDER_PROTO_READ_BUF_SIZE : 0,
    };
        ...
        bool do_receive)
{
    ...
    else if (bwr->read_consumed > KMHAL_HIDL_BINDER_PROTO_READ_BUF_SIZE)
...
#define try_read_advance(var) do {              \
    ...
    *p += sizeof((var));                        \
    ...
    case BR_REPLY: {
        ...
        struct kmhal_hidl_binder_tr_sg_args_out *const o =
            td->data.tr_sg.out_data;
